...
def main():
    ...
    # データの読み込みとデータセットの作成
    encoder = SeparatePhraseEncoder(tokenizer, seq_len, with_phrase=True)
    ...
    corrects, incorrects = test_model(model, bert_emb, test_loader, device)
    ...
    pd.to_pickle(  corrects, '../result/bert_siries_correct.pkl')
    pd.to_pickle(incorrects, '../result/bert_siries_incorrect.pkl')
