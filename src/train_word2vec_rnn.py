...
def main():
    ...
    # データの読み込みとデータセットの作成
    embed = Word2vecEmbedding(seq_len=seq_len)

    train_dataset = WordnetDataset(mode='train', num_data=num_train, transform=embed)
    valid_dataset = WordnetDataset(mode='valid', num_data=num_valid, transform=embed)
